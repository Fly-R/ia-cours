...
class YoloPrepareData:
    ...
    def prepare_new_dataset(
        self, dataset_path: str, ratios: list[float] = None, seed: int = 42
    ) -> YoloConfig:
        """
        ...
        YoloPrepareData.__move_annotations(
            os.listdir(yolo_config.images_train),
            annotations_path,
            yolo_config.labels_train,
        )
        YoloPrepareData.__move_annotations(
            os.listdir(yolo_config.images_val), annotations_path, yolo_config.labels_val
        )
        YoloPrepareData.__move_annotations(
            os.listdir(yolo_config.images_test),
            annotations_path,
            yolo_config.labels_test,
        )
    ...
    @staticmethod
    def __move_annotations(
        images: list[str], annotations_src: str, annotations_dest: str
    ) -> None:
        """
    ...
    def __download_annotations(self, annotations_path: str) -> None:
        ...
        :return:
        """
        zip_path = self.__dataset.export_annotation_file(
            AnnotationFileType.YOLO, annotations_path, use_id=self.__use_image_id
        )
